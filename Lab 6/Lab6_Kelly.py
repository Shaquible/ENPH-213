...
def IDFT(ck, n, tmin=0, tmax=2*np.pi):
    ...
    yj = 1/n*np.sum(ck*np.exp(1j*k*2*np.pi/n*j[:, None]), axis=1)
...
def DFTvariableTime(f, n, tmin, tmax, w, sigma):
    ...
    j = np.linspace(0, n-1, n, endpoint=True)
    tj = j*(tmax-tmin)/n+tmin

    yj = f(tj, w, sigma)
    # summing across all time values for each k (now in the j array)
    ck = np.sum(yj*np.exp(-1j*j[:, None]*j*2*np.pi/n), axis=1)
    return j, ck
...
def DFTshift(f, n, tmin, tmax, w, sigma):
    ...
    wshift = np.fft.fftshift(np.fft.fftfreq(n, (tmax-tmin)/n)*2*np.pi)
...
fig, ax = plt.subplots(1, 2, figsize=(10, 5))
ax[0].plot(t, yj, "r-", label="$\omega_p=10$")
ax[0].set_title("Double Frequency Resolution")
ax[0].set_xlabel("t")
ax[0].set_ylabel("y(t)")
ax[1].plot(wshift, abs(yshift), "ro-", label="$\omega_p=10$")
ax[1].set_xlabel("$\omega$")
...
def freqFilter(w, ck):
    ...
    max = abs(ck).max()
    # searching for indexs that have roughly the magnitude as max to account for floating point errors
    for i in range(len(w)):
        if round(abs(ck[i])) != round(max):
            ck[i] = 0
    return ck


# calculating the dft and plotting it and the original function
k, ck = DFTvariableTimeY1(y1, 200, 0, 8*np.pi, 4)
...
ax[0].set_xlabel("t")
ax[0].set_ylabel("y(t)")
ax[1].plot(k, abs(ck), "r-", label="no filter")
ax[1].set_xlabel("$\omega$")
ax[1].set_ylabel(r"$|\tilde{y}|$")
#ax[1].set_xlim(0, 50)
...
ax[0].plot(xs, np.real(ys), "b-", label="filtered")
